            ...
            if ("X" in dataAll[ii].loc[i, "管径或断面"]) and (dataAll[ii].loc[i, "管径或断面"] != "kong"):
                ...
                if (dataAll[ii].loc[i,"特征"] == "拐点" and dataAll[ii].loc[i, "附属物"] == "kong") :
                    ...
                    if count_flag == 2:
                        count_flag_r = 0 # 矩形标记
                        for j in range(0, len(data)):
                            if (data.loc[j, "管线点预编号"] == linkPoint and data.loc[
                                j, "连接点号"] == startPoint) and ("X" in data.loc[j, "管径或断面"]):
                                if count_flag_r < 2:
            ...
            if not ("X" in dataAll[ii].loc[i, "管径或断面"]):
                ...
                if ((dataAll[ii].loc[i,"特征"] == "拐点" and dataAll[ii].loc[i, "附属物"] == "kong") or
                    ...
                    for j in range(0, len(data)):
                        print(data.loc[j, "管线点预编号"])
                        print(linkPoint)
                        print(data.loc[j, "连接点号"])
                        print(startPoint)
                        if (data.loc[j, "管线点预编号"] == linkPoint and data.loc[j, "连接点号"] == startPoint) and ("X" in str(data.loc[j, "管径或断面"])):
                            count_flag = count_flag + 1
                    if count_flag == 2:
                        count_flag_c = 0  # 矩形标记
                        for j in range(0, len(data)):
                            if (data.loc[j, "管线点预编号"] == linkPoint and data.loc[j, "连接点号"] == startPoint) and (not ("X" in str(data.loc[j, "管径或断面"]))):
                                if count_flag_c < 2 :
